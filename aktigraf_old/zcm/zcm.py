...
def calc_zcm_raw(dir_path: str, periods_number: int, frequency: str, _start_date: str, _uuid: str, _format: str, _lvl = 0.15):
    """
    ...
    for _file in os.listdir(dir_path):
        limit += 1
        if limit < 2000 :#and int(_file[1:3]) > 66:
                ...
                while day < max_day:
                    ...
                    for c in chunks:
                        ...
                        for idx, val in enumerate(c):
                            ...
                            ni = c[idx]
                            if 0 < idx < len(c) - 1:
                                if (mx[pi] < threshold - _lvl and threshold + _lvl < mx[ni]) or (mx[ni] < threshold - _lvl and threshold + _lvl < mx[pi]):
                                    crossed += 1
                                    crossed_x += 1

                                if (my[pi] < threshold - _lvl and threshold + _lvl < my[ni]) or (my[ni] < threshold - _lvl and threshold + _lvl < my[pi]):
                                    crossed += 1
                                    crossed_y += 1

                                if (mz[pi] < threshold - _lvl and threshold + _lvl < mz[ni]) or (mz[ni] < threshold - _lvl and threshold + _lvl < mz[pi]):
                                    crossed += 1
                                    crossed_z += 1
            ...
            Path(f"Values_RAW_{dominator}/{_lvl}").mkdir(parents=True, exist_ok=True)
            with open(f"Values_RAW_{dominator}/{_lvl}/{name}_{str(_lvl)}.csv", 'w', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=headers)
                writer.writeheader()
                writer.writerows(list_to_write)

#calc_zcm_raw(dir_path3, periods_number, frequency, _start_date, _uuid, _format, 0.05)
