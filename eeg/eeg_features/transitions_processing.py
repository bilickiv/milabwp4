...
def _conditionalHomogeneityTest(_X, ns, l, alpha, verbose=True):
    ...
    if verbose:
        print(f"p: {p:.2e} | t: {T:.3f} | df: {df:.1f}")
    return_dict = {f"homogenity_p_{l}": p}
    return return_dict
...
def calculate_transition_metrics(ids, categories, transitions, limit_from, limit_to, possible_switches, mode=""):


    LIST_TO_WRITE = []
    LIST_TO_WRITE_HOMOGENITY = []
    ...
    for ix, tr in enumerate(transitions):
        ...
        LIST_TO_WRITE_HOMOGENITY.append(dict_to_write_homm)
        ...
        if not headers_shannon:
            ...
            headers_shannon += homm_keys

    Path(f"characteristics/sajat_absolute").mkdir(parents=True, exist_ok=True)
    with open(f"characteristics/absolute_sajatraBackfitting/eeg_transition_metrics{mode}.csv", 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=headers)
        writer.writeheader()
        writer.writerows(LIST_TO_WRITE)

    with open(f"characteristics/absolute_sajatraBackfitting/eeg_transition_homogenity_metric{mode}.csv", 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=headers_homm)
        writer.writeheader()
        writer.writerows(LIST_TO_WRITE_HOMOGENITY)

    with open(f"characteristics/absolute_sajatraBackfitting/eeg_transition_shannon_metric{mode}.csv", 'w', newline='') as csvfile:
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("characteristics/absolute_sajatraBackfitting/eeg_sequence_transitions.csv")
    ...
    """
    df = pd.read_csv("characteristics/sajat_absolute/eeg_sequence_transitions_sima.csv")
    ...
    calculate_transition_metrics(_ids, _categories, _transitions, limit_from, limit_to, possible_switches, "_sima")

    df = pd.read_csv("characteristics/sajat_absolute/eeg_sequence_transitions_eredo.csv")
    ...
    calculate_transition_metrics(_ids, _categories, _transitions, limit_from, limit_to, possible_switches, "_eredo")
